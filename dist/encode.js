...
export async function _internal_encode(wasmEncodeFn, { frames, width, height, fps, frameDurations, quality, repeat, resizeWidth, resizeHeight, progressCallback, }) {
    if (frames.length === 1) {
    ...
    const _frameDurations = frameDurations
        ...
        : undefined;
    const buffer = await wasmEncodeFn(framesBuffer, numOfFrames, width, height, fps, _frameDurations, quality, repeat, resizeWidth, resizeHeight, progressCallback);
    if (!buffer)
